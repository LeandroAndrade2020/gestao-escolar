...
<div class="container-fluid">
    ...
    <?php
        $sql = "select * from bimestre where rg_professor = '$rg' and escola = '$escola'' and serie = '$serie[0]' and turma = '$serie[1]'";
        $result = $conexao->query($sql);
        
        foreach($result as $consultas) {
            echo '<tr>';
                echo "<td class='text-center'>{$consultas['bimestre']}</td>";
                ...
                <td class='text-center' style='display: flex;'>
                    <form method='post' action='visualizar2.php'>
                        ...
                        <input name='conteudo' type='hidden' value='{$consultas['conteudo']}'>
                         <input name='data' type='hidden' value='{$consultas['bimestre']}'>
