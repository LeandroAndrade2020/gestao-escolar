...
<?php
  ...
  $interacao = $_POST['interacao'];
    ...
    $sqlOb = "select * from matricula where escola = '$nome_escola' and serie = '$serie[0]' and turma = '$serie[1]'  order by numero_chamada asc ";
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            . "<th> <select name=\"{$row['ra']}[]\" > <option>$interacao</option> <option>Facebook</option>  <option>Whatsapp</option>  <option>Outro</option>  <option>Classroom</option>   <option>Aula</option> <option>Impresso</option> </select> </th>"
